...
class ManiuplatorModel:
    ...
    def M(self, x):
        ...
        alpha = self.m1*(self.l1/2)**2 + self.I_1 +    self.m2*(self.l1**2+ (self.l2/2)**2) + self.I_2 + self.m3*(self.l1**2+ self.l2**2) + self.I_3
        beta = self.m2*self.l1*self.l2/2 + self.m3*self.l1*self.l2
        gamma = self.m2*(self.l2/2)**2 + self.I_2  + self.m3*self.l2**2 + self.I_3
    ...
    def C(self, x):
        q1, q2, q1_dot, q2_dot = x
        alpha = self.m1 * (self.l1 / 2) ** 2 + self.I_1 + self.m2 * (
                    self.l1 ** 2 + (self.l2 / 2) ** 2) + self.I_2 + self.m3 * (self.l1 ** 2 + self.l2 ** 2) + self.I_3
        beta = self.m2 * self.l1 * self.l2 / 2 + self.m3 * self.l1 * self.l2
        gamma = self.m2 * (self.l2 / 2) ** 2 + self.I_2 + self.m3 * self.l2 ** 2 + self.I_3
